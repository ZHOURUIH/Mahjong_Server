...
class txCommand;
class CommandSystem : public FrameComponent
{
public:
	CommandSystem(const string& name);
	...
	// 创建命令
	template<typename T>
	T* newCmd(T*& cmd, const string& file, int line, bool show = true, bool delay = false)
	{
	...
	void pushCommand(txCommand* cmd, CommandReceiver* cmdReceiver);
	// 延迟执行的命令都会在主线程中执行
	// delayExecute是命令延时执行的时间,默认为0.01,只有new出来的命令才能延时执行
	void pushDelayCommand(txCommand* cmd, CommandReceiver* cmdReceiver, float delayExecute = 0.001f);
