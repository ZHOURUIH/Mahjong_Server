...
//-------------------------------------------------------------------------------------------------------------------------------------------------------
// 常量数字定义
const int CLIENT_TEMP_BUFFER_SIZE = 11 * 1024;	// 客户端临时缓冲区大小,应该不小于单个消息包最大的大小
const int CLIENT_SEND_BUFFER_SIZE = 8 * 1024 * 1024;// 客户端发送数据缓冲区大小,8MB
const int CLIENT_RECV_BUFFER_SIZE = 8 * 1024 * 1024;// 客户端接收数据缓冲区大小,8MB
const int SERVER_RECV_BUFFER_SIZE = 8 * 1024 * 1024;	// 从socket接收数据时使用的缓冲区,8MB
