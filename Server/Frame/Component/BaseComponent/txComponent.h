...
{
public:
	txComponent(const string& typeName, const string& name);
	virtual ~txComponent(){ destroy(); }
	virtual void init(ComponentOwner* owner)
	{
	...
	virtual void attachOwnerParentComponent(ComponentOwner* owner, txComponent* parent, int childPos);
	...
	bool moveChildPos(const string& name, int destPos);
	txComponent* getChildComponent(const string& childName)
	{
	...
	// 获得成员变量
	ComponentOwner* getOwner()		{ return mComponentOwner; }
	...
	const string& getBaseType()	{ return mBaseType; }
	...
	const txMap<string, txComponent*>& getChildComponentMap() { return mChildComponentMap; }
	...
	virtual void notifyChildNameChanged(const string& oldName, txComponent* component);
protected:
	// 只是为子类提供一个公用的函数
...
protected:
	ComponentOwner* mComponentOwner;	// 该组件的拥有者
	string mBaseType;				// 基础组件类型,指定该组件属于什么基础类型的组件,如果不属于任何基础类型,则填实际组件类型
	string mType;					// 实际组件类型
	string mName;					// 组件名
	bool mActive;						// 是否激活组件
	bool mLockOneFrame;					// 是否将组件锁定一次
	txComponent* mParent;				// 父级组件
	txMap<string, txComponent*> mChildComponentMap;	// 用于查找组件
