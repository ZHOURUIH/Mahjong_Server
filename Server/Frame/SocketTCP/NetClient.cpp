...
mAsyncExecute(false)
{
	...
	memset(mIP, 0, 16);
	memcpy(mIP, ip, getMax(16, (int)strlen(ip)));
...
void NetClient::destroy()
{
	...
	if (mCharacterGUID != INVALID_ID)
	{
		...
		cmdOffline->mCharacterGUID = mCharacterGUID;
...
void NetClient::sendPacket(Packet* packet, bool autoDestroyPacket)
{
	...
	packet->writeToBuffer(mTempBuffer1, packetSize);
	// 写入类型,数据长度,消息内容
	int offset = 0;
	write(mTempBuffer0, CLIENT_TEMP_BUFFER_SIZE, offset, (int)packet->getPacketType());
	write(mTempBuffer0, CLIENT_TEMP_BUFFER_SIZE, offset, packetSize);
	writeBuffer(mTempBuffer0, CLIENT_TEMP_BUFFER_SIZE, offset, mTempBuffer1, packetSize);
...
PARSE_RESULT NetClient::parsePacket(int& index, Packet*& packet)
{
	...
	PACKET_TYPE type = (PACKET_TYPE)read<int>(mRecvStreamBuffer->getData(), mRecvStreamBuffer->getBufferSize(), index);
	int packetSize = read<int>(mRecvStreamBuffer->getData(), mRecvStreamBuffer->getBufferSize(), index);
	if (packetSize < 0)
	{
		return PR_ERROR;
	}
	...
	if (mRecvStreamBuffer->getDataLength() >= index + packetSize)
	{
		...
		if (!packet->readFromBuffer(mRecvStreamBuffer->getData() + index, packetSize))
		{
			// 解析错误
			NetServer::destroyPacket(packet);
			packet = NULL;
...
bool NetClient::sendPacketCheck(Packet* packet)
{
	...
	{
		clientError("临时缓冲区太小! send size : " + intToString(sendSize) + ", temp buffer size : " + intToString(CLIENT_TEMP_BUFFER_SIZE));
		return false;
	}
...
void NetClient::executeAllPacket()
{
	...
	swap(mReadExecuteIndex, mWriteExecuteIndex);
	...
	FOR(executeList, int i = 0; i < count; ++i)
	{
		...
		if (NetServer::getOutputLog() && packetReply->showInfo())
		{
			clientInfo("execute : type : " + intToString(packetReply->getPacketType()) + ", desc : " + packetReply->debugInfo());
		}
		packetReply->execute();
...
void NetClient::clientInfo(const string& info)
{
	LOG_INFO("IP : " + string(mIP) + ", 账号GUID : " + intToString(mAccountGUID) + ", 角色GUID : " + intToString(mCharacterGUID) + " || " + info);
}

void NetClient::clientError(const string& info)
{
	LOG_ERROR("IP : " + string(mIP)  + ", 账号GUID : " + intToString(mAccountGUID) + ", 角色GUID : " + intToString(mCharacterGUID) + " || " + info);
}